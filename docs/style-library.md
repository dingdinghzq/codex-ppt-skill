...
- For a style-anchor slide, inspect the internally accepted anchor image.
...
If the target filename already exists during active deck production, choose a clear suffixed name and continue. Ask whether to overwrite or merge only when style-library editing is the user's standalone task and no slide production is underway.
...
That list is what future upfront style selection reads before offering or using reusable style references.
