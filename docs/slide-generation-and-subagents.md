...
After upfront intake is complete and the outline, visual style, image backend, and style anchor have been selected or internally accepted, create final downstream artifacts if they do not already exist:

- `deck_spec.json`
- `prompts/slide_XX.json`
- `speech.md`

Do not create these final downstream artifacts before upfront intake is complete when material questions remain. Once production begins, create and update the final artifacts without additional approval pauses.

`deck_spec.json` must include `sample_generation_method` copied from the internally accepted style anchor before `prepare_slide_prompts.py` is run. The legacy field name remains for script compatibility. The helper copies that method into each `prompts/slide_XX.json` and into `slide_jobs.json`, so workers can see the exact backend, tool, mode, image context preparation, and output constraints used for the anchor.
...
If preparing prompts manually instead of using `prepare_slide_prompts.py`, still save each full slide job under `{base_dir}/{deck_name}/prompts/slide_XX.json` before generation. The saved job must include `prompt`, `out`, and `input_images`, including any deck-level accepted style-anchor reference and all slide-level source images with explicit role labels.

## Slide Generation With Workers Or Parent-Local Execution

If the current runtime already permits subagents, use one subagent per remaining slide image job up to the concurrency limit. If subagents are unavailable, generate the jobs parent-locally with the same selected backend and generation contract. Do not ask the user for worker authorization during intake or production.

Use the slide state scripts as the dispatch contract for both modes. A slide is not considered dispatched or complete until the relevant script records it. For parent-local generation, record dispatch with a stable agent id such as `parent-local`, then use the same id when recording the result.
...
- Ensure the accepted style-anchor slide is included in every non-anchor job as a style-only input image when available.
...
- Ensure `sample_generation_method` is present in `deck_spec.json`, every `prompts/slide_XX.json`, and `slide_jobs.json`; it must describe the exact backend/tool/mode used to generate the accepted style anchor.
- Mark the internally accepted style-anchor slide in `deck_spec.json` with `sample_approved: true` or `approved_sample: true` before running `prepare_slide_prompts.py`; these legacy keys cause the helper to record it as `accepted` when the final image file exists.
...
- In authorized parallel mode, spawn subagents with exactly one slide job each, up to `dispatch_slots_available`.
- Immediately after each successful spawn, run `record_slide_dispatch.py` with the real agent id and prompt path.
- In parent-local mode, run `record_slide_dispatch.py` with `--agent-id parent-local` before generating each job.
...
- Follow the `sample_generation_method` from the assigned job. Use the same tool family, generation/edit mode, image context preparation, and model/config details that produced the accepted style anchor.
...
- Treat the accepted style-anchor slide as style reference only.
...
If authorized parallel workers fail to spawn, continue parent-locally when the selected backend remains available. Report a blocker only when the backend or a strict input path is unavailable, not merely because worker delegation failed.
...
~/.codex-ppt-skill/.venv/bin/python {skill_root}/scripts/record_slide_result.py \
  ...
  --qa-note "Text readable; style matches the accepted style anchor."
```
...
- The accepted style-anchor slide should already have the correct `slide_XX.png` filename and should be reused directly.
...
- Before assembling, verify every expected `slide_XX.png` exists in `origin_image/`, there are no missing or extra final slide images, and `slide_job_status.py` shows all non-anchor slide jobs as `recorded`.
