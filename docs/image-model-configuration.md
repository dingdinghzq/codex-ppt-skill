...
Require configuration only when:
...
If CLI/API fallback is known to be required before production starts, check configuration before the upfront intake and include any missing credential or endpoint requirement in that one batch. After production begins, do not ask a new configuration question; report the exact blocker and required setting.
...
If Codex is being used through a GPT subscription and the built-in image tool is available, do not require separate `gpt-image-2` configuration.
