...
- Record the exact asset path or attachment name in `outline.md` for that slide, preferably as a Markdown image reference inside that slide's `Required images` list. Resolve ambiguous mapping in the single upfront intake; do not ask again after production begins.
...
- After generation, inspect the output yourself and repair any incorrect use of required figures. Mention any unresolved fidelity limitation in the final report without opening another approval round.
