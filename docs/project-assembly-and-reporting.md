...
Make sure `outline.md` reflects the final deck outline produced from upfront intake and inferred defaults. Do not recreate it from scratch here.
...
- The script only reads final images named like `slide_01.png`, `slide_02.png`, etc.; drafts and non-final reference files are ignored.
- Before running assembly, `slide_jobs.json` should show every generated slide as `recorded` and the internally accepted style-anchor slide as `accepted`. If any slide is `pending`, `dispatched`, or `blocked`, stop and report that state without asking another question.
...
- Number of slides
- Confirm which image backend was used and that every non-anchor slide result was recorded with `record_slide_result.py`.
